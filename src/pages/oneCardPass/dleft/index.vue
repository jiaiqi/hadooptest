<template>
	<div class="left-content">
		<div class="left-box">
			<databox :title="'医院列表'" :dheight="720">
				<!-- <personal :personalData="personalData" :username="username"></personal> -->
				<!-- <databox :title="'详情'" :dheight="280" :icon="'account'" :boxb="false">
			     	<ve-pie :data="starData" :extend="extend" :height="'250px'"></ve-pie>
			     	 <nodata :nodata="'一个Stars'" v-if="noStarData"></nodata>
				 </databox> -->
				 <itemView v-for="(item,index) in listData" :key="index" :selected="false" :itemData="item"></itemView>
				 <!-- <databox :title="'医院列表'" :dheight="280" :icon="'account'" :boxb="false">
			     	 
							
				 </databox> -->
				 <!-- <databox :title="'分类'" :dheight="280" :icon="'account'" :boxb="false">
...
export default {
  ...
  created(){
		this.listData = this.$chartData.hospitalList
  },
  methods:{
  	getData(username){
