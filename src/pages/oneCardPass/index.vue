<template>
	<div class="data-page">
		<dheader :titleText="'社保医疗一卡通融合平台'" :fontSize="'3rem'"></dheader>
		<topnav></topnav>
		<div class="data-content">
			...
			<div class="data-main">
				...
				<div class="main-center">
					<dcontent :numberData="numberData" :type="'left'" :lineData="lineData" :username="username"></dcontent>
				</div>
				<div class="main-center">
					<dcontentr :numberData="numberData" :type="'right'" :lineData="lineData" :username="username"></dcontentr>
				</div>
		...
		<pmodal :isShow="isShows" @sayhidden="sayhidden" v-if="isShows"/>
	</div>
</template>
...
import dcontentr from './dcontent'
import dright from './dright'
import pmodal from "../app/pmodal";
export default {
  components:{
  	dheader,
  	dleft,
  	dcontent,
  	dright,pmodal,dcontentr
  },
  data(){
  	return{
			isShow:false,
  ...
  created(){
  	let username = this.$route.params.user
		...
		this.lineData = this.$chartData.registered
  },
  methods:{
		sayhidden(sayhidden) {
